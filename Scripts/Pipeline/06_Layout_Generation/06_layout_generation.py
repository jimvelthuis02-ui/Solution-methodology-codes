...
def _profile_is_feasible_exact_fill(
    ...
    canonical_final_slot = max(
        (value for value in sorted(config_values) if value <= int(round(float(final_slot)))),
        default=None,
    )
    if canonical_final_slot is None:
        return False

    effective_profile = [float(value) for value in lower_slots] + [float(canonical_final_slot)]
...
def _generate_feasible_rack_profiles_cached(candidate_slot_sizes: tuple[float, ...]) -> tuple[tuple[float, ...], ...]:
    """Generate legal profiles in descending slot-size construction order.

    The profile family is built by maximizing the largest configured slot size first, then the next
    size, and so on. A legal topfill is only used as the final row needed to complete the exact
    754 cm stack for that underlying family; it is never treated as a separate family. Once a size
    has appeared in three generated profiles, later profiles containing that size are skipped until
    every configured size has reached the same minimum, thereby implementing the requested quota as a
    profile-order coverage rule rather than as a per-profile repeat count.
    """
    ...
    def profile_family_hits(profile: tuple[float, ...] | list[float]) -> set[int]:
        ...
        for value in profile:
            if float(value) <= 0.0:
                continue
            mapped = _effective_requirement_slot_size(float(value), profile, configured_sizes)
            if mapped in family_set:
                hits.add(mapped)
        return hits

    def try_emit_profile(lower_slots: list[float]) -> None:
        if not lower_slots:
            return

        lower_total = sum(lower_slots)
        lower_rows = len(lower_slots)
        final_value = common.MAX_USED_HEIGHT_BASE - lower_total - common.BEAM_HEIGHT * lower_rows
        if final_value <= 0.0:
            return
        ...
        candidate = tuple(sorted(lower_slots, reverse=True) + [float(rounded_final)])
    ...
    def recurse(size_index: int, lower_slots: list[float]) -> None:
        ...
        size = configured_sizes[size_index]
        max_count = min(max_count_cap, int(round(common.MAX_USED_HEIGHT_BASE // size)) + 2)
        for count in range(max_count, -1, -1):
            next_lower_slots = lower_slots + [float(size)] * count
            next_total = sum(next_lower_slots)
            remaining_sizes = configured_sizes[size_index + 1:]
            minimum_future = sum(remaining_sizes) if remaining_sizes else 0
            if remaining_sizes and next_total + minimum_future > common.MAX_USED_HEIGHT_BASE:
                continue
...
def _choose_profile_shortlist(
    ...
    search_limit = max(1, int(limit if limit is not None else PROFILE_CANDIDATE_LIMIT))
...
def _effective_requirement_slot_size(
    ...
    """Map a legal final topfill to the configured family it completes for exact-count accounting."""
    ...
    if rounded == profile_values[-1] and rounded not in set(config_values):
        canonical = max((value for value in config_values if value <= rounded), default=rounded)
        return canonical
    return rounded
...
def _effective_requirement_counts(
    ...
    counts_key = (profile_values, config_values)

    @lru_cache(maxsize=256)
    def _counts_cached(profile_key: tuple[int, ...], available_key: tuple[int, ...]) -> tuple[tuple[int, int], ...]:
        config_values_for_count = set(available_key)
        counts: Counter[int] = Counter({size: 0 for size in sorted(config_values_for_count)})
        for index, value in enumerate(profile_key):
            effective = value
            if index == len(profile_key) - 1 and len(profile_key) > 1 and value not in config_values_for_count:
                effective = max((size for size in sorted(config_values_for_count) if size <= value), default=value)
            counts[effective] += 1
        return tuple(sorted(counts.items()))

    return dict(_counts_cached(*counts_key))
...
def _effective_slot_size_for_summary(slot_size: float, column_slots: list[float], available_slot_sizes: SlotSizeSequence = None) -> int:
    ...
    A legal topfill such as 84 or 109 is not a standalone demand family; it represents the residual
    completion of the underlying configured family (for CFG_001, the 69 family). When aggregating
    the final layout distribution for reporting, we therefore map topfills back to the nearest lower
    configured representative size instead of counting them as extra demand slots.
    """
    rounded = int(round(float(slot_size)))
    if not column_slots:
        return rounded
    ...
    legal_topfills = set(_legal_topfill_values(available_slot_sizes or column_slots))
    if rounded in legal_topfills:
        candidates = [value for value in config_values if value < rounded]
        if candidates:
    ...
    for rack in sorted(rack_rows):
        ...
        for row in rows:
            ...
            if slot_size is not None:
                slot_counts[int(round(float(slot_size)))] += 1
            column = str(row.get("Column", "")).strip()
            if column and slot_size is not None:
...
def build_layout_generation() -> tuple[list[dict[str, str]], list[dict[str, str]], list[dict[str, str]]]:
    ...
    for config in configs:
        ...
        feasible_profile_pool = _generate_feasible_rack_profiles(config_slot_sizes or expansion_slot_sizes)
